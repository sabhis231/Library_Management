...
$(function () {
    ...
    var selectedBook = [];
    $.ajax({
        url: "../../FetchAllReturnedBook",
        method: "POST",
        success: function (result) {
            console.log(result);
            if (result.responseCode == 1) {
                bookArray = [...result.allReturnedBook];
...
var renderData = function (bookArray) {
    ...
    if (bookArray.length == 0) {
        data += "<tr>";
        data += "<td colspan='6' style='text-align: center;'>No Book returned by user.</td>";
    } else {
        for (var a of bookArray) {
            ...
            data += "<td>" + trasformDate(a.createdOn) + "</td>";
